...
{
	boost::stacktrace::safe_dump_to ("ban_node_backtrace.dump");
}
...
void assert_internal (const char * check_expr, const char * func, const char * file, unsigned int line, bool is_release_assert, std::string_view error_msg)
{
...
#ifdef _WIN32
	{
		...
		std::string filename = is_release_assert ? "ban_node_backtrace_release_assert.txt" : "ban_node_backtrace_assert.txt";
