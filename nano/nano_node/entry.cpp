	...
	if (ec == nano::error_cli::unknown_command)
	{
		...
		else if (vm.count ("compare_rep_weights"))
		{
			if (!nano::network_constants ().is_dev_network ())
			{
				...
				auto const outlier_threshold = std::max (nano::kBAN_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (nano::kBAN_ratio, mismatch_mean.number ());
				...
				// Log more data
				auto const log_threshold = nano::kBAN_ratio;
