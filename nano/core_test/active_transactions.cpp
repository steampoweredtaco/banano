...
TEST (active_transactions, inactive_votes_cache_existing_vote)
{
	...
	auto send = builder.send ()
				.previous (latest)
				.destination (key.pub)
				.balance (nano::genesis_amount - 100 * nano::kBAN_ratio)
				.sign (nano::dev_genesis_key.prv, nano::dev_genesis_key.pub)
				.work (*system.work.generate (latest))
				.build_shared ();
...
TEST (active_transactions, inactive_votes_cache_election_start)
{
	...
	auto send1 = send_block_builder.make_block ()
				 .previous (latest)
				 .destination (key1.pub)
				 .balance (nano::genesis_amount - 5000 * nano::kBAN_ratio)
				 .sign (nano::dev_genesis_key.prv, nano::dev_genesis_key.pub)
				 .work (*system.work.generate (latest))
				 .build_shared ();
	auto send2 = send_block_builder.make_block ()
				 ...
				 .balance (nano::genesis_amount - 10000 * nano::kBAN_ratio)
				 .sign (nano::dev_genesis_key.prv, nano::dev_genesis_key.pub)
				 .work (*system.work.generate (send1->hash ()))
				 .build_shared ();
...
TEST (active_transactions, vote_replays)
{
	...
	auto send1 = builder.make_block ()
				 ...
				 .previous (genesis.hash ())
				 .representative (nano::dev_genesis_key.pub)
				 .balance (nano::genesis_amount - nano::kBAN_ratio)
				 .link (key.pub)
				 .sign (nano::dev_genesis_key.prv, nano::dev_genesis_key.pub)
				 .work (*system.work.generate (genesis.hash ()))
				 .build_shared ();
	...
	ASSERT_EQ (nano::vote_code::replay, node.active.vote (vote_open1));
	ASSERT_EQ (nano::kBAN_ratio, node.ledger.weight (key.pub));

	auto send2 = builder.make_block ()
				 .account (key.pub)
				 .previous (open1->hash ())
				 .representative (key.pub)
				 .balance (nano::kBAN_ratio - 1)
				 .link (key.pub)
				 .sign (key.prv, key.pub)
...
TEST (active_transactions, fork_replacement_tally)
{
	...
	auto send_last = builder.make_block ()
					 ...
					 .previous (latest)
					 .representative (nano::dev_genesis_key.pub)
					 .balance (balance - 2 * nano::kBAN_ratio)
					 .link (key.pub)
					 .sign (nano::dev_genesis_key.prv, nano::dev_genesis_key.pub)
					 .work (*system.work.generate (latest))
					 .build_shared ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (latest)
					.representative (nano::dev_genesis_key.pub)
					.balance (balance - nano::kBAN_ratio - i)
					.link (key.pub)
					.sign (nano::dev_genesis_key.prv, nano::dev_genesis_key.pub)
					.work (*system.work.generate (latest))
					.build_shared ();
